import { useReducer, useRef, useEffect, useCallback, useMemo, MutableRefObject } from 'react'
...
export function useInterval(callback: any, delay: any) {
  const savedCallback = useRef() as MutableRefObject<HTMLInputElement>
  ...
  useEffect(() => {
    function tick() {
      const callbackFunction : any = savedCallback.current
      callbackFunction()
    }
...
function useProgress(isActive: any, config = {}) {
  ...
  }, [isActive, prevIsActive, progress])
